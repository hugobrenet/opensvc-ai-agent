...
func TestLiveChatCompletionsText(t *testing.T) {
	client := liveClient(t)
...
func TestLiveChatCompletionsToolCall(t *testing.T) {
	...
	request := llm.Request{Messages: []llm.Message{userMessage}, Tools: []llm.Tool{tool}}
...
func liveClient(t *testing.T) llm.Client {
	t.Helper()
	if os.Getenv("OPENSVC_AI_LLM_PROTOCOL") != config.LLMProtocolChatCompletions {
		t.Skip("live Chat Completions configuration is unavailable")
	}
